...
import { GEN1_COUNT, GEN2_COUNT, GEN3_COUNT, GEN4_COUNT } from '../services/pokeapi';
...
function createRouter(service?: PokemonService): Router {
  ...
  router.get('/', async (req: Request, res: Response) => {
    try {
      ...
      const gen = ['1', '2', '3', '4', '5'].includes(req.query.gen as string) ? req.query.gen : 'all';
      res.render('index', { pokemon, GEN1_COUNT, GEN2_COUNT, GEN3_COUNT, GEN4_COUNT, gen });
    } catch {
      res.status(500).render('index', { pokemon: [], GEN1_COUNT, GEN2_COUNT, GEN3_COUNT, GEN4_COUNT, gen: 'all' });
    }
  });
